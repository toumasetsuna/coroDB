...
//          return status;
//      }
//    io_prep_pread(&io, fd, scratch, n, static_cast<off_t>(offset));
//    iocbp = &io;
//
...
//    ssize_t read_size = events[0].res;
...
//    return status;
    ...
    PROMISE(Status) Cororead(uint64_t offset, size_t n, Slice* result,
                char* scratch) const  {
        thread_local int x=1;
        thread_local struct io_uring ring;
        thread_local std::vector<int> finished_queue(batch_size*3);
        thread_local Generator generator(batch_size*3);
        thread_local int min_batch=0;
        min_batch++;
        if(x==1){
            x=0;
            if (io_uring_queue_init(batch_size*3, &ring,  IORING_SETUP_SQPOLL) < 0) {
        ...
        io_uring_prep_read(sqe, fd, scratch, n, static_cast<off_t>(offset));
        int k=generator.get();
