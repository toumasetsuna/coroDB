...
#include "debug.h"
#include <sched.h>
#include "util/cache_size.h"
#include "mutex"
#define CHAR_BIT 8
namespace levelDB {
    class tcalloc {
        struct alignas(64) FrameNode {
            FrameNode *next;
            uint8_t entry_index;
            uint8_t padding[64 - sizeof(intptr_t) - sizeof(uint8_t)];
        };

        static_assert(sizeof(FrameNode) == 64);
        ...
        void *alloc(size_t byte_size) {
            ...
            if (frame_to_alloc == nullptr) {
                ...
                frame_to_alloc = reinterpret_cast<FrameNode *>(alloc_from_arena(
                        sizeof(FrameNode) + frame_size, 64));
...
// of the coroutine call stack.
        struct promise_base {
            ...
            void unhandled_exception() { std::__terminate(); }
    ...
    namespace task {
        ...
        template<>
        struct task<void>::promise_type : coro_task_private::promise_base {
...
inline void prefetch(const char* data, size_t size) {
    static const size_t cache_line_size_ = 64;
    for(size_t i = 0; i < size; i += cache_line_size_)
...
#define ADV_COROUTINE
#ifdef ADV_COROUTINE
#define STR(x) #x
#define PROMISE(t) levelDB::task::task<t>
#define RETURN co_return
#define AWAIT co_await
#define SUSPEND co_await std::suspend_always{}

    template<typename T>
    inline T sync_wait_coro(levelDB::task::task<T> &&coro_task) {
        coro_task.start();
    ...
    template<>
    inline void sync_wait_coro(levelDB::task::task<void> &&coro_task) {
        coro_task.start();
        while(!coro_task.done()) {
            coro_task.resume();
        }
    ...
    inline void sync_wait_void_coro(levelDB::task::task<void> &&coro_task) {
        coro_task.start();
        while(!coro_task.done()) {
            coro_task.resume();
        }
