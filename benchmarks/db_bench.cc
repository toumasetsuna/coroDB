...
//      deleterandom  -- delete N keys in random orders
...
static int FLAGS_write_buffer_size = -1;
 ...
 public:
  ...
  void FinishedSingleOp() {
    if (FLAGS_histogram) {
      ...
      hist_.Add(micros);
      if (micros > 20000) {
        std::fprintf(stdout, "long op: %.1f micros%30s\r", micros, "");
        std::fflush(stdout);
      }
      last_op_finish_ = now;
    }
    ...
    if (done_ >= next_report_) {
      if (next_report_ < 1000)
        ...
        next_report_ += 100000;
      std::fprintf(stdout, "... finished %d ops%30s\r", done_, "");
      std::fflush(stdout);
    }
  }
...
class Benchmark {
 ...
 private:
  ...
  void ReadRandom(ThreadState* thread) {
    ...
    for (int i = 0; i < reads_; i+=batch_size) {
        std::vector<ReadOptions> options(batch_size);
        std::vector<KeyBuffer> keyBuffers(batch_size);
        std::vector<Status> statusList(batch_size);
        for (int j = 0; j < batch_size; j++) {
            int const k = thread->rand.Uniform(FLAGS_num);
            keyBuffers[j].Set(k);
            keys[j]=keyBuffers[j].slice();
        }
        db_->MultiGet(options, keys, &values,&statusList);
        for(int j = 0; j < batch_size; j++){
            if(statusList[j].ok()){
                found++;
            }
  ...
  void ReadMissing(ThreadState* thread) {
      std::vector<ReadOptions> options;
    ...
    for (int i = 0; i < reads_; i+=batch_size) {
        ...
        for (int j = 0; j < batch_size; j++) {
            ...
            key.Set(k);
            keys.push_back(Slice(key.slice().data(), key.slice().size() - 1));
        }
        options.resize(keys.size());
        values.resize(keys.size());
        statusList.resize(keys.size());
        db_->MultiGet(options, keys, &values,&statusList);
        for(int j = 0; j < batch_size; j++)
            thread->stats.FinishedSingleOp();
...
//    if (io_uring_queue_init(batch_size*FLAGS_threads, &ring,  IORING_SETUP_SQPOLL) < 0) {
